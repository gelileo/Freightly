...
**部分**机械化——见下方"已知局限")是:**不信任 LLM 陈述一个在原文里无法验证存在的事实。**
...
warnings: list[str] = []`。`draft()` 会把 `warnings` 透传到 `DraftResult.warnings`,让信号
到达调用方/审批界面。

**已知局限(仅作用于 filled_slots)——在接入真实 Gemini 路径前需补齐。** 该校验只遍历
`raw.filled_slots`,不会扫描正文里 LLM **未**在 `filled_slots` 中报告的事实性词元。用
`FakeLlmClient` 时不会发生(它总是如实报告),且每份草稿都在强制人工审批门之后,故本无界面切片
无实际暴露。在 `GeminiLlmClient` 接入真实起草路径之前,应补上**正文扫描**(例如正文里出现
`\b60\d{9}\b`(BOL)/PRO 形态但不在 `source_text` 中 → 告警)、把子串/替换改为**按词边界**匹配
(当前一个恰为真实值子串的捏造值会被误接受)、并加固 `GeminiLlmClient` 的 JSON 解析。已记录于
`knowledge/log.md`。
