...
```bash
python3 manage.py                 # 交互式菜单:启停各应用、seed/reset、模式切换(推荐)
# …或直接:
python3 scripts/seed_demo.py      # 在 ./hs.db 建演示账号(幂等)
python3 scripts/serve_local.py    # 在 http://127.0.0.1:8000 提供 /(代理)+ /customer + /api
```

`manage.py` **在每次渲染菜单前先显示状态块(进程状态 + URL + DB 计数)**,再等待选择。启动已针对
"静默孤儿进程"陷阱做了加固:先做**端口预检**,若 `PORT` 已被占用则拒绝启动 web(并给出 `kill …` 提示);
且用 `proc.poll()` + 就绪探测确认子进程**确实在监听**,而非用 `os.kill(pid,0)`——后者会把未回收的
**僵尸(zombie)**子进程误判为"存活",从而在进程已崩溃、旧服务器仍以旧路由应答时谎报启动成功。状态块
也会标出同样的孤儿情形(web 未被跟踪但 `PORT` 被占用)。`_alive()` 亦已识别僵尸(`ps STAT` 为 `Z`),
使状态显示同样可靠。
