...
Then log in on the running site (python3 scripts/serve_local.py):
  • Agent console  http://127.0.0.1:8000/          → email op@example.com / password agent-demo
...
def main():
    ...
    if conn.execute("SELECT 1 FROM users WHERE id='op'").fetchone():
        print(f"demo data already present in {db_path}. Agent: op@example.com / '{DEMO_AGENT_PW}'"
    ...
    repo.create_user(conn, "Agent Operator", "email", "op@example.com", id="op")
    ...
    mailbox = os.environ.get("SMTP_ADDRESS", "hs@example.com")
    ...
    print(f"  Agent console (/):        email 'op@example.com'  password '{DEMO_AGENT_PW}'")
