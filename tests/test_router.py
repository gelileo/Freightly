...
def test_add_agent_operator_creates_agent_user_with_password():
    ...
    assert out["login"] == "jane@example.com" and out["role"] == "operator"
    assert out["temp_password"]                         # generated + returned
    assert repo.is_member(c, "jane@example.com", "a1")
    from app import auth
    assert auth.login_password(c, "jane@example.com", out["temp_password"]) is not None
    # an explicit password is used and not echoed back
    out2 = router.add_agent_operator(c, agent_org_id="a1", name="Bob", email="bob@example.com",
                                     password="set-by-admin", role="admin")
    assert out2["temp_password"] is None and out2["role"] == "admin"
    assert auth.login_password(c, "bob@example.com", "set-by-admin") is not None
